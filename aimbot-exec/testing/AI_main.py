...
def control_mouse(a, b, fps_var, ranges, rate, go_fire, win_class, move_rx, move_ry, down_time, up_time):
    ...
    if fps_var and arr[17] and arr[11] and arr[4]:
        ...
        x0 = {
            'CrossFire': a / 2.719 * (client_ratio[0] / (4/3)) / fps_factorx,  # 32
        ...
        y0 = {
            'CrossFire': b / 2.719 * (client_ratio[0] / (4/3)) / fps_factory,  # 32
...
def main():
    ...
    # 寻找读取游戏窗口类型并确认截取位置
    window_class_name, window_hwnd_name, window_outer_hwnd, test_win[0] = get_window_info()
    ...
    while True:
        ...
        if str(win32gui.GetForegroundWindow()) in (str(window_hwnd_name) + str(window_outer_hwnd)) and not test_win[0]:
            if arr[4]:  # 是否需要控制鼠标
                if arr[15] == 1:  # 主武器
...
arr = Array('i', range(22))  # 进程间分享数据
mouse_detect_proc = Process(target=detection, args=(arr,))  # 鼠标检测进程
...
test_win, shoot_times, recoil_control, show_fps, DPI_Var, client_ratio = [False], [0], [0], [1], [1], [1]
window_class_name, window_hwnd_name, window_outer_hwnd = None, None, None
